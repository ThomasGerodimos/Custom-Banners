...
//Αρχικοποίηση της ημερομηνίας έναρξης προβολής
// Hook into save_post action
add_action('save_post', 'set_post_live_start_date', 10, 3);

function set_post_live_start_date($post_id, $post, $update) {
    // Check if the post is being created for the first time
    if ($update || wp_is_post_revision($post_id)) {
        return; // Don't proceed if post is being updated or is a revision
    }

    // Check if the post type is 'live'
    if ($post->post_type === 'live') {
        // Get the post creation date
        $post_creation_date = $post->post_date;

        // Update the start date meta field with the post creation date
        update_post_meta($post_id, 'start_date', $post_creation_date);
    }
}
...
function custom_live_banner_shortcode() {
    ...
    if ($live_query->have_posts()) :
        while ($live_query->have_posts()) : $live_query->the_post();
			...
			echo $plugin_url . '/Live-icon_20x20.png'
